let cnv, vid1, vid2, vid3, vid3Gfx, snd1, snd2, snd3, ding, pipe, drawState, vocoder, delay, calibrateBtn, slider;
...
function buttonOne() {
    calibrateBtn = createButton('are you sure about this?');
    calibrateBtn.position(20, 20);
    calibrateBtn.mousePressed(goToStateTwo);
    ding.play();
}

function buttonTwo() {
    calibrateBtn = createButton('alright fine. *ahem* minimalism in 2025 be like:');
    calibrateBtn.position(20, 20);
    calibrateBtn.mousePressed(goToStateThree);
    ding.play();
}

function buttonThree() {
    calibrateBtn = createButton('oh you wanna give it a shot? fine, go ahead');
    calibrateBtn.position(20, 20);
    calibrateBtn.mousePressed(() => {
        ...
        slider.mouseOver(() => {
            calibrateBtn.remove();
            calibrateBtn = createButton('just take it easy okay?');
            ding.play();
            calibrateBtn.position(20, 20);
            slider.mouseOver(() => {});
        });
...
function buttonFour() {
    calibrateBtn = createButton('oh my god');
    calibrateBtn.position(20, 20);
    setTimeout(() => {
        calibrateBtn.remove();
    }, 5000);
...
function buttonFive() {
    calibrateBtn = createButton('why would you do that. what were you thinking i literally just warned you');
    calibrateBtn.position(20, 20);
    setTimeout(() => {
        calibrateBtn.remove();  // have to use braces so btn is referenced correctly
    }, 5000);
...
function buttonSix() {
    calibrateBtn = createButton('you should really clean up the mess you made');
    delay.process(snd2, 0.8, 0.8, 2300);
    calibrateBtn.position(20, 20);
    calibrateBtn.mousePressed(goToStateOne);
    ding.play();
}

function goToStateOne() {
    if (calibrateBtn !== undefined) calibrateBtn.remove();
